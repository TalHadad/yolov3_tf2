...
def non_max_suppression(inputs, model_size, max_output_size, max_output_size_per_class, iou_threshold, confidence_threshold):
    '''leverage the Tensorflow's built-in NMS function (tf.image.combine_non_max_suppression).'''
...
def resize_image(inputs, modelsize):
    '''resize the image to fit with the model's size'''
...
def draw_outputs(img, boxes, objectness, classes, nums, class_names):
    ...
    for i in range(nums):
        ...
        img = cv2.putText(img, '{} {:.4f}'.format(
            class_names[int(classes[i])], objectness[i]), (x1y1), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
    return img
